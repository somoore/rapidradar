...
def handle_event(helper: AWSHelper, event, notification_app, webhook_urls, slack_oauth_token, sender_email, alert_suppression_resource_tag_key_value, alert_suppression_permission_set_tag_key_value, send_logs_to_azure, customer_id, shared_key, log_type, s3_buckets_table_name, pagerduty_helper, is_pd_integration_type_restapi):
    ...
    if 'errorCode' not in event and 'requestParameters' in event:
        ...
        try:
            ...
            if 'Tagging' in request_params and 'TagSet' in request_params['Tagging'] and 'Tag' in request_params['Tagging']['TagSet']:
                tags = request_params['Tagging']['TagSet']['Tag']
                if isinstance(tags, list):
                    for tag in tags:
                        if tag['Key'] == alert_suppression_tag_key and tag['Value'] == alert_suppression_tag_value:
                            found_suppression_tag = True
                elif isinstance(tags, dict):
                    if tags['Key'] == alert_suppression_tag_key and tags['Value'] == alert_suppression_tag_value:
                        found_suppression_tag = True
            ...
            elif (is_public_bucket or found_public_objects) and not previously_notifications_compressed and found_suppression_tag and found_admin_suppression_tag:
                if not s3_buckets_table_ops.store(helper.account_id, helper.region, bucket_name, is_bucket_encrypted, is_public_bucket, found_public_objects, found_suppression_tag, pagerduty_incident_id=updated_incident_ids if is_pd_integration_type_restapi else None, pagerduty_dedup_keys=None if is_pd_integration_type_restapi else updated_incident_ids):
                    LOGGER.error("Could not add metadata for S3 Bucket %s in Account ID=%s and Region=%s", bucket_name, helper.account_id, helper.region)
                azure_data['Event'] = f"User {helper.iam_user} has tagged S3 Bucket {bucket_name} with {alert_suppression_tag_key}={alert_suppression_tag_value} tag which will silence the notifications for this resource"
                alerts_handler = Alert(None, [], is_pd_integration_type_restapi, False, messenger, send_logs_to_azure, azure_data, customer_id, shared_key, log_type)
                alerts_handler.handler('notifications_suppression_message', alert_args, email_messenger, slack_bot)

            elif (is_public_bucket or found_public_objects) and not previously_notifications_compressed and found_suppression_tag and not found_admin_suppression_tag:
                azure_data['Event'] = f"User {helper.iam_user} added {alert_suppression_tag_key}={alert_suppression_tag_value} tag to S3 Bucket {bucket_name} but they do not have permission to enable or disable notifications, that's why you'll continue to receive alerts for this resource"
                alerts_handler = Alert(None, [], is_pd_integration_type_restapi, False, messenger, send_logs_to_azure, azure_data, customer_id, shared_key, log_type)
        ...
        LOGGER.info("Tags on S3 Bucket were not added for some reason")
