...
def handle_event(helper: AWSHelper, event, notification_app, webhook_urls, slack_oauth_token, sender_email, send_logs_to_azure, customer_id, shared_key, log_type, s3_buckets_table_name, pagerduty_helper, incident_finding_types, is_pd_integration_type_restapi):
    ...
    if 'errorCode' not in event and 'requestParameters' in event:
        ...
        try:
            ...
            grants = request_params['AccessControlPolicy']['AccessControlList']['Grant']
                ...
                for grant in grants:
                    if 'URI' in grant['Grantee']:
                        if grant['Grantee']['URI'] in ['http://acs.amazonaws.com/groups/global/AllUsers', 'http://acs.amazonaws.com/groups/global/AuthenticatedUsers']:
                            is_bucket_acls_public = True
            elif isinstance(grants, dict):
                if 'URI' in grants['Grantee']:
                    if grant['Grantee']['URI'] in ['http://acs.amazonaws.com/groups/global/AllUsers', 'http://acs.amazonaws.com/groups/global/AuthenticatedUsers']:
                        is_bucket_acls_public = True
            ...
            if is_bucket_acls_public:
                ...
                LOGGER.info("S3 Bucket ACLs not public.")
        except Exception as error:
            LOGGER.error(str(error))
    else:
        LOGGER.info("Bucket ACL addition failed for some reason")
