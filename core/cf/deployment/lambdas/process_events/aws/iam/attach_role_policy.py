    ...
    if 'errorCode' not in event and 'requestParameters' in event and 'roleName' in event['requestParameters']:
        ...
        try:
            ...
            if is_role_attached_to_ec2:
                is_attached_role_policy_overly_permissive = Helper().is_attached_role_policy_overly_permissive(request_params['policyArn'])
                if is_attached_role_policy_overly_permissive:
                    azure_data = {
                    ...
                    if not iam_utils.found_iam_role_bypass_tag(role_name, ec2_instance_iam_role_detection_bypass_tag_key):
                        ...
                        if auto_remediate_over_permissive_roles:
                            if not iam_utils.detach_role_policy(role_name, request_params['policyArn']):
                                LOGGER.error("Could not detach IAM Role Policy %s from IAM Role %s", request_params['policyArn'], role_name)
                            else:
                                if len(email_resources_block) > 0:
                                    ...
                                    alert_args = {
                                        ...
                                        "policy_name": request_params['policyArn']
                                    }
                                    azure_data['Severity'] = severity
                                    azure_data['Event'] = f"User {helper.iam_user} created/attached an over-permissive policy named {request_params['policyArn']} to IAM Role {role_name} which is attached to EC2 Instance. The policy has been detached by our threatOps automation"
                            ...
                            if len(email_resources_block) > 0:
                                ...
                                azure_data['Severity'] = severity
                                azure_data['Event'] = f"User {helper.iam_user} created/attached an over-permissive policy named {request_params['policyArn']} to IAM Role {role_name} which is attached to EC2 Instance"
                        ...
                        if len(email_resources_block) > 0:
                            ...
                            azure_data['Severity'] = severity
                            azure_data['Event'] = f"User {helper.iam_user} created/attached an over-permissive policy {request_params['policyArn']} to IAM Role {role_name} which is attached to EC2 Instance. But, this policy was not deleted/detached because Admin has used bypass tag on IAM Role"
                            alerts_handler = Alert(pagerduty_helper, incident_finding_types, is_pd_integration_type_restapi, False, messenger, send_logs_to_azure, azure_data, customer_id, shared_key, log_type)
