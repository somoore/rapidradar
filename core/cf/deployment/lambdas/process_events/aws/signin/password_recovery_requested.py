...
def handle_event(helper: AWSHelper, event, notification_app, webhook_urls, ip_history_table, deploy_ip_tracker_project, send_logs_to_azure, customer_id, shared_key, log_type, pagerduty_helper, incident_finding_types, is_pd_integration_type_restapi):
    if 'responseElements' in event and 'PasswordRecoveryRequested' in event['responseElements']:
        ...
        if response_elements['PasswordRecoveryRequested'] == 'Success':
            ...
            azure_data = {
                ...
                "Event": f"Someone requested for password change for ROOT user.{azure_message} Please verify the legitimacy of the password change event"
            }
