...
def handle_event(event, helper: AWSHelper, active_regions, enable_ec2_instance_configurator, deployment_targets, exclude_accounts, ssm_document_name):
    if 'errorCode' not in event and 'errorMessage' not in event:
        if 'serviceEventDetails' in event and 'createManagedAccountStatus' in event['serviceEventDetails']:
            managed_account_status_details = event['serviceEventDetails']['createManagedAccountStatus']
            if managed_account_status_details['state'] == 'SUCCEEDED':
                new_account_id = managed_account_status_details['account']['accountId']
                ...
                LOGGER.info("Account Creation State: %s", managed_account_status_details['state'])
    else:
