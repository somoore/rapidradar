...
def handle_event(helper: AWSHelper, event, notification_app, webhook_urls, slack_oauth_token, sender_email, send_logs_to_azure, customer_id, shared_key, log_type, project_name, tag_ami, tag_ami_using_tag_template_for_tf_deployment, tags_key_value_ami, send_missing_tags_notification_ami, pagerduty_helper, incident_finding_types, is_pd_integration_type_restapi):
    if 'errorCode' not in event and 'responseElements' in event and 'requestParameters' in event:
        ...
        try:
            ami_id = response_elements['imageId']
            ...
            if tag_ami:
                ...
                if tag_ami_using_tag_template_for_tf_deployment and is_tf_deployment:
                    decrypted_value = json.loads(ssm_utils.get_decrypted_value(f'/{project_name}/AMI_TAG_TEMPLATE'))
                    for key, value in decrypted_value.items():
                        if value != f"< {key.upper()} >":
                            use_params_tags = False
                            if not ec2_utils.add_tags_to_ec2_resource(ami_id, [{'Key': key, 'Value': value}]):
                                LOGGER.error("Could not add tag %s to AMI %s in Account=%s and Region=%s", key, ami_id, helper.account_id, helper.region)
                if use_params_tags:
                    for tag_key_value in tags_key_value_ami:
                        key_value = tag_key_value.split('=')
                        if key_value[0] not in ami_tag_keys:
                            if not ec2_utils.add_tags_to_ec2_resource(ami_id, [{'Key': key_value[0], 'Value': key_value[1]}]):
                                LOGGER.error("Could not add tag %s to AMI %s in Account=%s and Region=%s", key_value[0], ami_id, helper.account_id, helper.region)

            if send_missing_tags_notification_ami and not tag_ami:
                ami_tags_key_value_missing = []
                for tag in tags_key_value_ami:
                    tag_key = tag.split('=')[0]
                    if tag_key not in ami_tag_keys:
                        ami_tags_key_value_missing.append(tag)
                if ami_tags_key_value_missing:
                    severity = 'Low'
                    alert_args = {
                        "severity": severity,
                        ...
                        "resource_id": ami_id,
                        "tags": ami_tags_key_value_missing
                    }
                    azure_data = {
                        "Severity": severity,
                        ...
                        "Event": f"User {helper.iam_user} created AMI {ami_id} without proper tags"
                    }
                    ...
                    LOGGER.info("Image was created with proper tags")
        except Exception as error:
            LOGGER.error(str(error))
    else:
        LOGGER.info("Image was not created for some reason")
