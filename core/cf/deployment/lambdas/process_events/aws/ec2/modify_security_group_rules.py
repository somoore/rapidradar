...
def handle_event(helper: AWSHelper, event, notification_app, webhook_urls, slack_oauth_token, sender_email, send_logs_to_azure, customer_id, shared_key, log_type, table_name, ec2_security_group_ingress_remote_access_ports, ec2_security_group_ingress_traffic_ports, ec2_security_group_ingress_ignore_ports, loadbalancer_security_group_ingress_remote_access_ports, loadbalancer_security_group_ingress_traffic_ports, loadbalancer_security_group_ingress_ignore_ports, target_arn, auto_delete_all_traffic_sg_rule, auto_remediate_launch_wizard_sg, auto_remediate_remote_access_ports, auto_remediate_traffic_ports, pagerduty_helper, incident_finding_types, is_pd_integration_type_restapi):
    if 'errorCode' not in event and 'requestParameters' in event:
        ...
        try:
            if 'ModifySecurityGroupRulesRequest' in request_params and 'SecurityGroupRule' in request_params['ModifySecurityGroupRulesRequest'] and 'SecurityGroupRule' in request_params['ModifySecurityGroupRulesRequest']['SecurityGroupRule']:
                security_group_id = request_params['ModifySecurityGroupRulesRequest']['GroupId']
                ...
                if group_name is not None:
                    if not group_name.startswith('launch-wizard-') or not auto_remediate_launch_wizard_sg:
                        ...
                        rule_details = request_params['ModifySecurityGroupRulesRequest']['SecurityGroupRule']['SecurityGroupRule']
                        if 'CidrIpv4' in rule_details and rule_details['CidrIpv4'] == '0.0.0.0/0':
                            ip_protocol = rule_details['IpProtocol']
                            is_all_traffic_open = Helper().is_all_traffic_port(ip_protocol)
                            if is_all_traffic_open:
                                rule_details['FromPort'] = -1
                                rule_details['ToPort'] = -1
                            port = rule_details['FromPort'] if rule_details['FromPort']==rule_details['ToPort'] else f"{rule_details['FromPort']}-{rule_details['ToPort']}"
                            security_group_rule_id = request_params['ModifySecurityGroupRulesRequest']['SecurityGroupRule']['SecurityGroupRuleId']
                            ...
                            if security_group_open_ports[0]:
                                ...
                                for item in security_group_open_ports[1]:
                                    if not Helper().matches_given_ports(item['Port'], item['Protocol'], ec2_security_group_ingress_ignore_ports, True) and not Helper().matches_given_ports(item['Port'], item['Protocol'], loadbalancer_security_group_ingress_ignore_ports, True):
                                        is_new_port = False
                                        port_userip = {}
                                        if item['RuleId'] == security_group_rule_id:
                                            is_new_port = True
                                            is_critical_finding = False
                                            is_high_finding = False
                                            is_medium_finding = False
                                            is_low_finding = False
                                            port_userip = {"Port": port, "Protocol": ip_protocol, "UserIpAddress": helper.user_ip_address}
                                            if is_all_traffic_open:
                                                if is_attached:
                                            ...
                                            if is_critical_finding or is_high_finding or is_medium_finding or is_low_finding:
                                                ...
                                                if not found_suppression_tag:
                                                    ...
                                                    if pagerduty_incident_id:
                                                        updated_incident_ids.append(pagerduty_incident_id)
                                        else:
                                ...
                                LOGGER.info("Not sending any notification since no ports are open in security group %s", security_group_id)
                        else:
                            LOGGER.info("CidrIpv4 block not found in events request parameters")
                    else:
                        LOGGER.info("Is a Launch Wizard Security Group and auto-remediation is enabled. Skipping...")
        except Exception as error:
            LOGGER.error(str(error))
    else:
        LOGGER.info("Security Group Rule was not modified for some reason")
