        ...
        try:
            if 'resourcesSet' in request_params and 'tagSet' in request_params and 'items' in request_params['resourcesSet'] and 'items' in request_params['tagSet']:
                    ...
                    if item['resourceId'].startswith("sg-"):
                        ...
                        for tag in request_params['tagSet']['items']:
                            if tag['key'] == alert_suppression_tag_key and tag['value'] == alert_suppression_tag_value:
                                ...
                                if found_admin_suppression_tag:
                                    ...
                                    if not SecurityGroupsData(security_groups_table).store(helper.account_id, helper.region, item['resourceId'], scan_records[0]['port']['SS'], scan_records[0]['attached']['S'], found_suppression_tag, pagerduty_incident_id=updated_incident_ids if is_pd_integration_type_restapi else None, pagerduty_dedup_keys=None if is_pd_integration_type_restapi else updated_incident_ids):
                                        LOGGER.error("Could not add metadata for Security Group %s in Account ID=%s and Region=%s", item['resourceId'], helper.account_id, helper.region)
                ...
                if not previously_notifications_compressed and found_suppression_tag and found_admin_suppression_tag:
                    azure_data['Event'] = f"User {helper.iam_user} has tagged {resource_type} {resource_id} with {alert_suppression_tag_key}={alert_suppression_tag_value} tag which will silence the notifications for this resource"
                    ...
                    azure_data['Event'] = f"User {helper.iam_user} added {alert_suppression_tag_key}={alert_suppression_tag_value} tag to {resource_type} {resource_id} but they do not have permission to enable or disable notifications, that's why you'll continue to receive alerts for this resource"
                    alerts_handler = Alert(None, [], is_pd_integration_type_restapi, False, messenger, send_logs_to_azure, azure_data, customer_id, shared_key, log_type)
                ...
                LOGGER.info("ResourceSet or TagSet not found in event's Request Parameters")
        except Exception as error:
            LOGGER.error(str(error))
    else:
        LOGGER.info("Tags were not added to resource for some reason")
