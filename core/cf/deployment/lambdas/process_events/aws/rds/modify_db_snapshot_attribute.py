...
def handle_event(helper: AWSHelper, event, notification_app, webhook_urls, slack_oauth_token, sender_email, send_logs_to_azure, customer_id, shared_key, log_type, auto_remediate_public_rds_snapshots, pagerduty_helper, incident_finding_types, is_pd_integration_type_restapi):
    ...
    if 'errorCode' not in event and 'errorMessage' not in event and 'requestParameters' in event:
        ...
        db_snapshot_id = request_params['dBSnapshotIdentifier']
        ...
        if request_params['attributeName'] == 'restore':
            ...
            if is_snapshot_public:
                ...
                if auto_remediate_public_rds_snapshots:
                    ...
                    if not rds_utils.make_public_db_snapshot_private(db_snapshot_id, False):
                        LOGGER.error("Could not make public RDS DB Snapshot %s private", db_snapshot_id)
                else:
                    severity = 'Critical'
                alert_args = {
                    "severity": severity,
                ...
                alert_args = {
                    "severity": severity,
                    "iam_user": helper.iam_user,
                    "resource_type": "RDS DB Instance Snapshot",
                ...
                azure_data = {
                    ...
                    "Event": f"User {helper.iam_user} made RDS DB Snapshot with ID {db_snapshot_id} public{' but since, auto-remediation is on for public RDS Snapshots, it has automatically been made PRIVATE' if auto_remediate_public_rds_snapshots else ''}"
                }
