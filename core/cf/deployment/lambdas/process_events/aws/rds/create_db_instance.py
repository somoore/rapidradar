...
def handle_event(helper: AWSHelper, event, event_time, sender_email, notification_app, webhook_urls, slack_oauth_token, active_resources_table, send_logs_to_azure, customer_id, shared_key, log_type, project_name, tag_rds, tag_rds_using_tag_template_for_tf_deployment, rds_tags_key_value, send_missing_tags_notification_rds_instances, deploy_cmdb_project, unencrypted_rds_creation_blocked, unencrypted_rds_creation_blocked_bypass_tag_key, alert_on_unencrypted_rds_creation_bypass, rds_cluster_instance_creation_blocked_wo_certain_tags, rds_cluster_instance_creation_scp_tag_keys, pagerduty_helper, incident_finding_types, is_pd_integration_type_restapi):
    ...
    if 'errorCode' in event and event['errorCode'] == 'AccessDenied' and 'errorMessage' in event and 'explicit deny in a service control policy' in event['errorMessage']:
        if unencrypted_rds_creation_blocked or rds_cluster_instance_creation_blocked_wo_certain_tags:
            ...
            azure_data['Event'] = f"User {helper.iam_user} tried to created an RDS DB Instance {azure_message} which was blocked by a service control policy."
        ...
        current_state = response_elements['dBInstanceStatus']
        ...
        rds_instance_tag_keys = [ tag['key'] for tag in response_elements['tagList'] ]
        ...
        if 'dBClusterIdentifier' in response_elements:
            LOGGER.info("Instance is part of a DB Cluster. Skipping...")
        else:
            ...
            if not is_rds_encrypted:
                ...
                azure_data['Severity'] = severity
                azure_data['Event'] = f"User {helper.iam_user} created an RDS DB Instance {db_identifier} without encryption enabled{' against SCP applied on your Organization using bypass tag.' if found_encrypted_rds_bypass_tag else '. Please add encryption to this RDS DB Instance by creating a snapshot of it, and then creating an encrypted copy of that snapshot and then restore a DB instance from the encrypted snapshot.'}"
            ...
            if deploy_cmdb_project:
                instance_class, rds_engine, storage_type = rds_utils.get_rds_cost_details(resource_type, db_identifier)
                hourly_cost = pricing_utils.get_rds_hourly_cost(instance_class, rds_engine, storage_type)
                ...
                if not deploy_method_key_exists:
                    ...
                    rds_utils.add_tags_to_rds(helper.account_id, f"db:{db_identifier}", [{'Key': 'DeployMethod', 'Value': deploy_method}])
                if not deployed_by_key_exists:
                    rds_utils.add_tags_to_rds(helper.account_id, f"db:{db_identifier}", [{'Key': 'DeployedBy', 'Value': event_by}])
                if not team_key_exists:
                    team='unknown'
                    rds_utils.add_tags_to_rds(helper.account_id, f"db:{db_identifier}", [{'Key': 'Team', 'Value': team}])
                if not ActiveResourcesData(active_resources_table).store(helper.account_id, account_name, helper.region, deploy_method, team, event_by, db_identifier, instance_class, resource_type, current_state, '', event_time, event_time, hourly_cost, daily_cost, monthly_cost, cost_type, platform, is_instance_ssm_managed, is_imdsv2_enabled, all_tags if all_tags else [""]):
                    LOGGER.error("Could not store data for RDS DB Instance %s in Account=%s and Region=%s", db_identifier, helper.account_id, helper.region)
                ...
                if tag_rds_using_tag_template_for_tf_deployment and is_tf_deployment:
                    ...
                    for key, value in decrypted_value.items():
                        if value != f"< {key.upper()} >":
                            ...
                            if not rds_utils.add_tags_to_rds(helper.account_id, f"db:{db_identifier}", [{'Key': key, 'Value': value}]):
                                LOGGER.error("Could not add tag %s to RDS DB Instance %s", key, db_identifier)
                if use_params_tags:
                    for rds_tag in rds_tags_key_value:
                        ...
                        if deployed_by_key_exists and key_value[0] in ['DeployedBy', 'deployedBy', 'Deployedby', 'deployedby']:
                            continue
                        if not rds_utils.add_tags_to_rds(helper.account_id, f"db:{db_identifier}", [{'Key': key_value[0], 'Value': key_value[1]}]):
                            LOGGER.error("Could not add tag %s to RDS DB Instance %s", key_value[0], db_identifier)

            if send_missing_tags_notification_rds_instances and not tag_rds:
                rds_tags_key_value_missing = []
                for tag in rds_tags_key_value:
                    tag_key = tag.split('=')[0]
                    if tag_key not in rds_instance_tag_keys:
                ...
                if rds_tags_key_value_missing:
                    ...
                    azure_data['Severity'] = severity
                    azure_data['Event'] = f"User {helper.iam_user} created an RDS Instance with ID {db_identifier} without proper tags"
                    alerts_handler = Alert(pagerduty_helper, incident_finding_types, is_pd_integration_type_restapi, False, messenger, send_logs_to_azure, azure_data, customer_id, shared_key, log_type)
                    alerts_handler.handler('resource_creation_without_tags_message', alert_args, email_messenger, slack_bot)
                else:
                    LOGGER.info("RDS Instance was created with proper tags")
    else:
